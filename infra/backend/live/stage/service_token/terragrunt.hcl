...
include "common" {
  path   = "${dirname(find_in_parent_folders("root.hcl"))}/backend/live/common/service_token.hcl"
...
terraform {
  ...
  before_hook "prevent_mod_token" {
    ...
    execute  = feature.modify_service_token.value ? ["bash", "-c", "echo 'Modifying service token.'"] : [
      "bash", "-c",
      "echo 'Modifying service token is skipped, as modify_service_token feature is set to false.' && exit 1"
    ]
  }
}
